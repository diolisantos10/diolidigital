    ...
    <div className="bg-white border border-[#E5E5E2] rounded-[10px] shadow-[0_1px_3px_rgba(0,0,0,0.04)] overflow-hidden mb-6">
      <div className="px-5 py-3.5 border-b border-[#F0F0ED]">
        <div className="flex items-center gap-2">
          <h2 className="text-[14px] font-semibold text-[#1C1C1A]">Conectar Inteligências Artificiais</h2>
          <span className="text-[10px] bg-[#DCFCE7] text-[#16A34A] px-2 py-0.5 rounded font-bold">CHAVES REAIS · CRIPTOGRAFADAS</span>
        </div>
        ...
        <div className="flex items-center gap-2.5 w-[210px] shrink-0">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <span className="text-[13px] font-semibold text-[#1C1C1A]">{meta.name}</span>
              <StatusDot loading={loading} configured={configured} tested={tested} />
            </div>
            <div className="text-[10px] text-[#9B9B95]">{meta.tagline}</div>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-[11px] font-semibold text-[#6B6B65]">Chave de API</label>
                <a
                  href={meta.keyUrl}
                  target="_blank"
              ...
              />
              <p className="text-[10px] text-[#9B9B95]">Onde encontrar: {meta.keyHelp}</p>

              <div className="flex items-center gap-2 pt-1">
                <select
                  value={model}
                  onChange={(e) => setModel(e.target.value)}
                  className="border border-[#E8E8E4] rounded-[7px] px-2.5 py-1.5 text-[12px] text-[#1C1C1A] bg-white focus:outline-none"
                >
                  {meta.models.map((m) => (
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="text-[12px] text-[#6B6B65]">
                <span className="font-medium text-[#1C1C1A]">Chave conectada:</span>{" "}
                <span className="font-mono">{status?.hint}</span>
                {status?.model && <span className="text-[#9B9B95]"> · {status.model}</span>}
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <button
                  onClick={handleTest}
                  disabled={busy !== ""}
                  className="px-3 py-1.5 text-[12px] font-medium border border-[#E8E8E4] text-[#6B6B65] rounded-[7px] hover:bg-[#F0F0ED] disabled:opacity-50"
                >
                  {busy === "testing" ? "Testando…" : "⚡ Testar"}
                </button>
                <button
                  onClick={() => setEditing(true)}
                  className="px-3 py-1.5 text-[12px] font-medium text-[#6B6B65] hover:text-[#1C1C1A]"
                >
                  Trocar chave
                </button>
                <button
                  ...
                  disabled={busy !== ""}
                  className="px-2.5 py-1.5 text-[12px] font-medium text-[#DC2626] hover:bg-[#FEE2E2] rounded-[7px] disabled:opacity-50"
                >
                  Remover
                </button>
              </div>
...
function StatusDot({ loading, configured, tested }: { loading: boolean; configured: boolean; tested: boolean }) {
  if (loading) return <span className="w-2 h-2 rounded-full bg-[#D4D4D0]" />;
  ...
  if (configured) {
    return (
      <span className="inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded-[4px] bg-[#FEF3C7] text-[#D97706]">
        ● Chave salva
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded-[4px] bg-[#F0F0ED] text-[#9B9B95]">
      ○ Não conectado
    </span>
  );
}
