    ...
    <div className="fixed inset-0 z-[90]">
      ...
      <aside className="absolute right-0 top-0 h-full w-[400px] max-w-[92vw] bg-white shadow-[-8px_0_40px_rgba(0,0,0,0.12)] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)] shrink-0">
          <div>
            <p className="text-[10px] font-semibold text-[var(--navy)] uppercase tracking-[0.06em]">
              Explicação da tela
            </p>
            <h2 className="text-[14px] font-semibold text-[var(--text-primary)]">{screenTitle}</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          {explanation.sections.map((sec) => (
            <div key={sec.title}>
              <h3 className="text-[11px] font-semibold text-[var(--text-muted)] uppercase tracking-[0.05em] mb-1.5">
                {sec.title}
              </h3>
              <ul className="space-y-1.5">
                {sec.items.map((item, i) => (
                  <li key={i} className="flex items-start gap-2 text-[12px] text-[var(--text-primary)] leading-relaxed">
                    <span className="text-[var(--navy)] shrink-0">·</span>
                    {item}
                  </li>
                ))}
              </ul>
          ...
          {explanation.attention.length > 0 && (
            <div>
              <h3 className="text-[11px] font-semibold text-[var(--warning)] uppercase tracking-[0.05em] mb-1.5">
                Precisa de atenção
              </h3>
              <ul className="space-y-1.5">
                {explanation.attention.map((item, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-2 text-[12px] text-[#92400E] leading-relaxed bg-[var(--warning-bg)] border border-[#FDE68A] rounded-[7px] px-3 py-2"
                  >
          ...
          {explanation.firstSteps.length > 0 && (
            <div>
              <h3 className="text-[11px] font-semibold text-[var(--success)] uppercase tracking-[0.05em] mb-1.5">
                Faça primeiro
              </h3>
              <ol className="space-y-1.5">
                {explanation.firstSteps.map((item, i) => (
                  <li key={i} className="flex items-start gap-2 text-[12px] text-[var(--text-primary)] leading-relaxed">
                    <span className="shrink-0 w-[18px] h-[18px] rounded-full bg-[#F0FDF4] border border-[#BBF7D0] text-[var(--success)] text-[10px] font-bold flex items-center justify-center mt-[1px]">
                      {i + 1}
                    </span>
                    {item}
                  </li>
