...
export default function ApprovalSaveToast({
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[100] max-w-[380px]">
      {saving && (
        <div className="flex items-center gap-2.5 bg-[var(--text-primary)] border border-white/[0.1] rounded-[10px] px-4 py-3 shadow-xl">
          <div className="w-3.5 h-3.5 rounded-full border-2 border-[var(--navy)] border-t-transparent animate-spin shrink-0" />
          <span className="text-[12px] text-[var(--text-subtle)]">Salvando aprovação no banco…</span>
        </div>
      )}
      {error && !saving && (
        <div className="bg-[#2A1212] border border-[var(--danger)]/40 rounded-[10px] px-4 py-3 shadow-xl">
          <div className="flex items-start gap-2.5">
            <span className="text-[var(--danger)] text-[13px] leading-none mt-0.5">✗</span>
