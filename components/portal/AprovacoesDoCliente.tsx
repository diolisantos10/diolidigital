...
// ── Ditado por voz — falar em vez de digitar ─────────────────────────────────
// Pedido do CEO (05/08/2026): "na parte da devolutiva lá na aprovação, eu
// gostaria que tivesse essa ferramenta de áudio que transcreve, eu não gosto de
// digitar."
//
// TRÊS TRAVAS que o pedido carrega e que a implementação não pode afrouxar:
//   1. Falar PREENCHE o campo. Enviar continua sendo um clique separado — nada
//      sai da mão do cliente sozinho.
//   2. O texto é EDITÁVEL: a transcrição cai no mesmo textarea, e ele corrige
//      antes de mandar. Transcrição erra; mandar erro para o cliente é criar
//      mal-entendido em nome da agência.
//   3. Falha de ditado NUNCA bloqueia a decisão — o campo de texto continua
//      inteiro. Sem suporte, o botão não existe (botão que não faz nada é pior
//      que botão ausente).
...
function useDitado({ token, onTexto }: { token: string; onTexto: (texto: string) => void }) {
  ...
  // Suporte só é sabido no navegador. Começar em `false` mantém o primeiro
  // render do cliente igual ao do servidor (sem erro de hidratação).
  useEffect(() => { setSuportado(suportaDitado()); }, []);
