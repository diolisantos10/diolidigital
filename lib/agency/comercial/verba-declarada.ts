...
import { PLANOS_PUBLICOS, precoEmReais } from "../planos";
...
export function confrontoDeVerba(
  ...
  // PLANOS_PUBLICOS, não PLANOS: esta lista chega ao PROSPECT (texto do briefing
  // público). Um plano com `exposicao: "interno"` (o Performance, não vendável
  // desde 03/08) nunca pode ser oferecido aqui — filtrar `PLANOS` ofereceria a
  // um prospect com teto alto um degrau que a casa não pode vender.
  const cabemNaVerba = PLANOS_PUBLICOS.filter((p) => p.preco <= teto)
