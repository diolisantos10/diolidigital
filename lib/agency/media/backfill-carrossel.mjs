...
/**
 * Nome produzido pela esteira: `carrossel-<postId>-<posição>.<ext>`.
 *
 * A extensão virou alternativa em 08/08/2026: a peça final passou a sair em
 * JPEG (o Instagram só aceita JPEG — ver `lib/integrations/meta/formato-de-midia.ts`)
 * e as telas gravadas ANTES daquele dia continuam `.png` no banco, para sempre.
 * Reconhecer só uma das duas deixaria este backfill cego para metade do acervo —
 * e cegueira aqui não aparece como erro: aparece como tela "não casada", que
 * alguém teria de casar no olho. Grupo não-capturante para os índices 1 e 2
 * continuarem sendo postId e posição.
 */
export const PADRAO_ESTEIRA = /^carrossel-(.+)-(\d+)\.(?:png|jpe?g)$/i;
 ...
 *
 * ── Por que "ambiguo" existe (07/08/2026), e por que ele vem ANTES de tudo ──
 * Medido em produção: os 6 carrosséis da Foocci estavam CERTOS e COMPLETOS —
 * as 6 telas v4 de 06/08, capa como tela 1. E o ensaio do backfill classificava
 * os 6 como "reparar", propondo acrescentar 5 telas a cada um.
 *
 ...
 * `carrossel-c1-tela-2-v3.png` e `foocci-carrossel1-tela2-v4-2026-08-06.png`
 * dizem os dois "carrossel 1, tela 2". O plano saía com 11 telas por post e
 * DUAS candidatas para cada posição. Como o que já estava gravado (as 6 novas)
 * é subconjunto do plano, `perderia` era falso e o reparo parecia legítimo:
 * teria montado seis carrosséis Frankenstein, alternando arte velha e nova, em
 * nome de um cliente pagante — e ainda estourando o teto de 10 da Meta.
 *
 * Duas imagens diferentes disputando a MESMA posição não é incompletude: é
 * AMBIGUIDADE. Incompletude o código resolve sozinho; ambiguidade é decisão de
 * gente, porque a máquina não tem como saber qual jogo é o aprovado. Vem antes
 * de todas as outras decisões porque, com posições em duplicata, `urls` já
 * nasce sem significado — inclusive para "sobrescrever" com `--force`.
 *
 * @returns {{acao: "ligar"|"reparar"|"sobrescrever"|"ja-tem-telas"|"sem-telas-suficientes"|"ambiguo",
 *            urls: string[], telasAtuais: number, faltando: number, posicoesEmDisputa: number[]}}
 */
export function decidirGravacao(post, { force = false } = {}) {
  const telas = post.telas ?? [];
  const urls = telas.map((t) => `/api/media/${t.assetId}`);
  const atuais = Array.isArray(post.telasAtuais) ? post.telasAtuais : [];
  const base = { urls, telasAtuais: atuais.length, faltando: 0, posicoesEmDisputa: [] };

  // ── AMBIGUIDADE: duas artes para a mesma tela. Ninguém escreve nada. ──────
  const vistas = new Set();
  const disputadas = new Set();
  for (const t of telas) {
    if (vistas.has(t.pos)) disputadas.add(t.pos);
    vistas.add(t.pos);
  }
  if (disputadas.size > 0) {
    return { ...base, acao: "ambiguo", posicoesEmDisputa: [...disputadas].sort((a, b) => a - b) };
  }
