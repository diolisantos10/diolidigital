...
import type { SimulationScenario, SimulationRun, PersonaAnswers } from "./types";
...
function detectQuestionTopic(text: string): keyof PersonaAnswers {
  ...
  if (/seu nome|nome do.{0,10}negócio|nome.{0,10}empresa|qual.{0,10}nome/i.test(t))
    return "identity";
  if (/e-mail|email|contato.*email|email.*contato/i.test(t))
    return "email";
  if (/whatsapp|telefone|celular|número.{0,10}contato/i.test(t))
    return "phone";
  if (/orçamento|investimento|budget|quanto.{0,10}(tem|você|planejou)|faixa.{0,10}(invest|preço)|estimativa.*está/i.test(t))
    return "budget";
  if (/objetivo|resultado|quer.{0,15}(conseguir|alcançar)|principal.*meta|foco.{0,10}(rede|digital)/i.test(t))
    return "objective";
  if (/tráfego pago|anúncios|ads|campanhas.{0,10}pag/i.test(t))
    return "traffic";
  if (/identidade visual|logo|marca.*visual|branding/i.test(t))
    return "branding";
  if (/posts|postagens|frequência|vezes.{0,10}semana|quantas.{0,10}(vezes|posts)|ritmo/i.test(t))
    return "frequency";
  if (/social media|redes sociais|instagram|facebook|tiktok|linkedin/i.test(t))
    return "service";
  return "default";
}

export function runSDRSimulation(scenario: SimulationScenario): SimulationRun {
  ...
  try {
    ...
    const handoff =
      state.sdr.qualificationScore >= 4
