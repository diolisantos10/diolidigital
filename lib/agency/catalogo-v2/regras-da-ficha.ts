...
//
// ─── A CONDIÇÃO EM QUE ESSA PROMESSA VALE: o cache é POR PROCESSO ──────────
//
// `cache`, logo abaixo, é um `Map` que vive pelo tempo do processo Node — não
// pelo tempo do deploy. Em PRODUÇÃO isso não muda a promessa: deploy novo é
// processo novo, o `Map` nasce vazio e a primeira leitura já pega a ficha
// atualizada. Em DESENVOLVIMENTO (`npm run dev`), o processo é o mesmo do
// início da sessão: editar a ficha NÃO reflete na próxima chamada enquanto o
// processo não reiniciar — é preciso derrubar e subir o `npm run dev` de
// novo para o agente vestir a edição. "Editou a ficha, subiu o deploy: o
// agente já vestiu" é verdade em produção; em dev, sem reiniciar, é falsa.
//
