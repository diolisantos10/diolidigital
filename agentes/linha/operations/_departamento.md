# Ficha de departamento — Operações, Sistemas e Segurança (`operations`)
...
**Manter integrações, automações, credenciais, segurança e continuidade.** Entrega ao próximo da esteira: serviços disponíveis, integrações saudáveis, alertas e recuperação comprovada.

## Risco de referência

Alto — é a fundação; erro aqui derruba todos os outros. O risco individual de cada função está na ficha dela; o
dono de negócio (CEO) ajusta quando quiser.

## A hierarquia, para não restar dúvida

```
CEO → Diretor → Gerente Geral → **Gerente de Operações, Sistemas e Segurança** (`manager-operacoes`) → funções desta pasta
```

Toda demanda deste departamento entra pelo **Gerente de Operações, Sistemas e Segurança**, que recebe do Gerente
...
|---|---|---|
| `manager-operacoes` | Gerente de Operações, Sistemas e Segurança | Médio |
| `integration-manager` | Agente Gestor de Integrações | Alto |
| `credentials-and-access` | Agente de Credenciais e Acessos | Crítico |
| `scheduler-and-recovery` | Agente Scheduler e Recovery | Alto |
| `observability` | Agente de Observabilidade | Médio |
| `security-and-audit` | Agente de Segurança e Auditoria | Crítico |
| `backup-and-continuity` | Agente de Backup e Continuidade | Alto |
