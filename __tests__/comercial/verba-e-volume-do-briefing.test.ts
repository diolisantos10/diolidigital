...
describe("volume zerado não vira orçamento", () => {
  ...
  it("volume de verdade destrava e escolhe o plano pelo que foi pedido", () => {
    ...
    // A tabela oficial (lib/agency/planos.ts) tem seis degraus; nenhum cobre
    // 56 peças/mês, então `detectPackage` devolve o TOPO cadenciado: Crescimento
    // (18 peças/mês). "Plano Premium" não existe — era um dos cinco planos-
    // fantasma que esta branch eliminou (ver cabeçalho de live-calculator.ts).
    const e = computeEstimate(
      escopoDoCityJobs({ social: { platforms: ["Instagram"], postsPerWeek: 14 } }),
    );

    expect(e.travadaPor).toBeUndefined();
    // Prova TOPO, não piso: teria de falhar se `detectPackage` devolvesse
    // Pulso ou Ritmo em vez do degrau mais caro da escada.
    expect(e.items.map((i) => i.label)).toContain("Plano Crescimento");
    expect(e.items.map((i) => i.label)).not.toContain("Plano Pulso");
    expect(e.items.map((i) => i.label)).not.toContain("Plano Ritmo");
...
describe("verba declarada não pode ser ignorada em silêncio", () => {
  ...
  it("compara contra o valor que o cliente REALMENTE pagaria, com desconto aplicado", () => {
    ...
    // não existe mais.
    //
    // Cenário real (tabela oficial, lib/agency/planos.ts): 2,5 posts/semana =
    // 10 peças/mês → Plano Presença, R$ 790 cheio. Teto declarado R$ 500
    // ("entre R$ 150 e R$ 500"). Cheio: 790 > 500, estoura. Com 40% de desconto
    // (o teto de `live-calculator.ts`): 790 × 0,6 = R$ 474, que cabe nos R$ 500.
    // "Essencial, piso R$ 600" não existe mais — era um dos planos-fantasma
    // eliminados desta base (ver cabeçalho de live-calculator.ts).
    const e = computeEstimate(
      escopoDoCityJobs({
        social: { platforms: ["Instagram"], postsPerWeek: 2.5 },
        branding: undefined,
        budgetRange: "entre R$ 150 e R$ 500",
    ...
    expect(e.items.map((i) => i.label)).toContain("Plano Presença");
