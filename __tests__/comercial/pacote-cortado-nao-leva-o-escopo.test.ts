...
describe("o pacote cortado no teto de tokens não leva o briefing junto", () => {
  it("salva os números que o cliente disse quando o corte cai dentro do escopo", async () => {
    ...
    // Antes deste conserto o turno inteiro virava `parse_error` e o `scope`
    // abaixo era `undefined` — os 14 posts/semana que o cliente acabara de
    // falar sumiam. Isso continua valendo: o escopo sobrevive palavra por
    // palavra, corte ou não.
    //
    // RECONCILIAÇÃO DE 16/08 — o que mudou foi o `ok`, e o porquê é a ordem do
    // JSON. Esta sessão inverteu o prompt de propósito (`scope` primeiro,
    // `reply` por último) para o corte cair na fala, não no dado. Esta fixture
    // é o caso raro em que o corte ainda assim caiu no `scope` (a fala, escrita
    // antes, saiu inteira) — mas o guarda não distingue os dois casos: qualquer
    // `stop_reason: "max_tokens"` já marca a fala como não confiável, mesmo
    // quando o texto do `reply` chegou completo. Era `ok: true` com a fala
    // repetida de volta; agora é `ok: false`, sem fala nenhuma, com o mesmo
    // escopo de sempre.
  ...
  it("os guardas continuam valendo sobre o escopo salvo do remendo", async () => {
    ...
    // RECONCILIAÇÃO DE 16/08: era `ok: true` — a heurística antiga lia
    // "scope" presente no JSON remendado como prova de que a fala tinha
    // fechado antes do corte. Sob a ordem nova do prompt (scope primeiro,
    // reply por último) essa prova deixou de valer: fala vinda de remendo é
    // SEMPRE barrada, escopo presente ou não. As travas abaixo são o que não
    // mudou — o escopo resgatado passa pelas mesmas regras de sempre.
    expect(corpo.ok).toBe(false);
...
describe("o diário para de misturar as duas causas de parse_error", () => {
  it("corte no teto é registrado como truncado — o remédio é o teto", async () => {
    ...
    // RECONCILIAÇÃO DE 16/08: o rótulo canônico é `truncado`, não
    // `parse_error_truncado` — a tabela de explicações em português
    // (`registro-da-conversa.ts`) já indexa por `truncado`/`malformado`, e o
    // prefixo `parse_error_` é jargão de código numa linha que o CEO lê.
    expect((await res.json()).reason).toBe("truncado");
  });
  ...
  it("formato quebrado é registrado como formato — o remédio é o prompt", async () => {
    ...
    // RECONCILIAÇÃO DE 16/08: o rótulo canônico é `malformado`, não
    // `parse_error_formato` — mesmo motivo do teste acima.
    expect((await res.json()).reason).toBe("malformado");
    ...
    expect(barrada).toContain("malformado");
  });
});
