...
| **A manchete da esteira nomeia o pedido menor.** Com uma direção pendente, `etapa` e `oQueEsperamosDeVoce` falavam só dos 5 pedidos de material. ⚠️ **Corrijo um diagnóstico meu:** cheguei a anotar que a direção "nunca aparecia" — aparece: `direcao: {pedeAprovacao: true}` está no MESMO payload. O defeito é menor do que eu escrevi: a frase de resumo não reflete a decisão que trava, não a ausência do botão | é escolha de qual pendência manda na frase; mexer sem ver a tela seria trocar um texto por outro no escuro | agência (portal) |
