---
titulo: "Google Ads — abuso da rede de publicidade"
url: https://support.google.com/adspolicy/answer/6020954?hl=pt-BR
capturado_em: 2026-08-13
hash: caa0aee36609663f
---

> Documento oficial capturado da plataforma. A fonte é a URL acima;
> este arquivo é a cópia de trabalho da biblioteca. Não edite à mão.

Abuso da rede de publicidade

O Google oferece versões traduzidas da Central de Ajuda, mas elas não têm a intenção de alterar o conteúdo das nossas políticas. A versão em inglês é o idioma oficial que usamos para aplicar essas políticas. Se quiser ver este artigo em outra língua, confira o menu suspenso de idiomas na parte de baixo da página.

Os usuários do Display & Video 360 precisam obedecer a essa política do Google Ads. Acesse a Central de Ajuda do Display & Video 360 para conferir outras restrições.

Os anúncios em toda a Rede do Google precisam ser úteis, variados, relevantes e seguros para os usuários. O Google Ads não permite que os anunciantes veiculem publicidade, conteúdo ou destinos que tentem enganar ou burlar nossos processos de revisão.

Veja abaixo alguns exemplos de conteúdo que deve ser evitado nos seus anúncios. Saiba o que acontece quando nossas políticas são violadas.

Neste artigo
Software malicioso
Sites comprometidos
Software indesejado
Vantagem injusta
Anúncio com conteúdo evasivo
Fraude de sistema
Políticas de spam para pesquisa na Web
Software malicioso

Malware é um software usado para causar danos ou conseguir acesso não autorizado a um computador, dispositivo ou rede.

Estes requisitos se aplicam aos seus anúncios e a qualquer software hospedado ou vinculado ao seu site ou app, seja o software promovido ou não pela rede de publicidade do Google.

Saiba mais sobre a política de software malicioso.

Violações dessa política são tratadas com seriedade e consideradas infrações graves. Uma violação grave das políticas do Google Ads é considerada tão séria que pode ser ilegal ou causar prejuízos significativos aos nossos usuários. Para determinar se um anunciante ou destino viola a política mencionada aqui, o Google analisa informações de várias fontes, como seu anúncio, site e contas, além de fontes externas. Se forem identificadas violações dessa política, suas contas do Google Ads serão suspensas imediatamente, sem aviso prévio, e você não poderá mais anunciar na plataforma. Se você acredita que houve um erro e que não violou nossa política, envie uma contestação e explique o motivo. As contas só são restabelecidas em circunstâncias justificáveis e quando há um motivo plausível. Por isso, a contestação precisa ser escrita de maneira honesta, clara e fundamentada. Saiba mais sobre contas suspensas.

Sites comprometidos

Um site comprometido é um domínio ou destino cujo código foi manipulado sem o conhecimento do proprietário ou operador para beneficiar terceiros, geralmente de forma prejudicial aos usuários.

Saiba mais sobre a política de sites comprometidos.

Software indesejado

Qualquer software hospedado ou vinculado ao seu site ou app precisa oferecer uma experiência positiva aos usuários. A política de software indesejado do Google descreve algumas características básicas dos softwares que são possivelmente prejudiciais para a experiência do usuário. Elas estão listadas abaixo:

São enganosos e fazem uma proposta de valor que não é atendida.
Tentam enganar os usuários para que eles os instalem ou se instalam junto com outro programa.
Não informam ao usuário todas as funções principais e significativas que têm.
Afetam o sistema do usuário de formas inesperadas.
São de difícil remoção.
Coletam ou transmitem informações particulares sem o conhecimento do usuário.
São agrupados com outros softwares, e a presença deles não é divulgada.

Saiba mais sobre a política de software indesejado.

Vantagem injusta

Não é permitido:

 Usar a Rede do Google para conseguir uma vantagem injusta no tráfego em relação a outros participantes no leilão

Saiba mais sobre a política de vantagem injusta.

Anúncio com conteúdo evasivo

Não é permitido:

 Manipulação de componentes do anúncio, como texto, imagem, vídeos, domínio ou subdomínios, para tentar burlar a detecção ou aplicação da política.

Saiba mais sobre a política de anúncios com conteúdo evasivo.

Fraude de sistema

Não é permitido:

 Adotar ou tentar adotar práticas que burlem ou interfiram nos sistemas e processos de publicidade do Google.

Saiba mais sobre a política de fraude de sistema.

Violações dessa política são tratadas com seriedade e consideradas infrações graves. Uma violação grave das políticas do Google Ads é considerada tão séria que pode ser ilegal ou causar prejuízos significativos aos nossos usuários. Para determinar se um anunciante ou destino viola a política mencionada aqui, o Google analisa informações de várias fontes, como seu anúncio, site e contas, além de fontes externas. Se forem identificadas violações dessa política, suas contas do Google Ads serão suspensas imediatamente, sem aviso prévio, e você não poderá mais anunciar na plataforma. Se você acredita que houve um erro e que não violou nossa política, envie uma contestação e explique o motivo. As contas só são restabelecidas em circunstâncias justificáveis e quando há um motivo plausível. Por isso, a contestação precisa ser escrita de maneira honesta, clara e fundamentada. Saiba mais sobre contas suspensas.

Políticas contra spam da Pesquisa Google na Web

Não é permitido:

 Envolvimento em práticas que violem as Políticas contra spam da Pesquisa Google na Web

Saiba mais sobre as políticas contra spam da Pesquisa Google na Web.

Precisa de ajuda?

Se tiver dúvidas sobre as políticas, confira as diretrizes delas e os exemplos ou fale com o suporte do Google Ads.

Esta página pode ter conteúdo que foi traduzido com tecnologia de IA. As traduções de IA podem conter erros.
Envie feedback sobre este artigo
Isso foi útil?
Como podemos melhorá-lo?
Enviar