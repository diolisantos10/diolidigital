...
Essa API oferece recursos nos canais Alfa e Beta. Os produtos Alfa e Beta podem ter suporte limitado, e as mudanças realizadas neles podem não ser compatíveis com outras versões Alfa e Beta.

Alfa: os recursos estão em um estágio inicial de pré-lançamento. Tentamos informar você sobre as próximas mudanças, mas pode haver alterações interruptivas antes do lançamento público da API.

Beta: não são esperadas mudanças importantes neste canal.
...
Gerencie as chaves secretas do Measurement Protocol.
...
Gerencie as vinculações entre as propriedades do Google Analytics e os projetos do Firebase.
Gerencie as vinculações entre propriedades do Google Analytics e contas do Google Ads.
...
Gerenciar permissões de usuário para uma hierarquia de contas e propriedades do Google Analytics (Alpha).
Gerar relatórios de acesso a dados.
Gerenciar públicos-alvo (Alfa).
Gerenciar conjuntos de dados abertos (Alfa).
...
Esse método retorna o campo accountTicketId que precisa ser incluído no URL dos Termos de Serviço (TOS):

https://analytics.google.com/analytics/web/?provisioningSignup=false#/termsofservice/ACCOUNT_TICKET_ID

Quando um usuário acessa o URL dos Termos de Serviço e aceita os Termos de Serviço, a criação de uma conta do Google Analytics é concluída. Consulte o exemplo de provisionamento de contas.
...
Gerenciamento de segredos do Measurement Protocol
...
Descontinuado: use o recurso e os métodos KeyEvent.
...
Propostas para vincular uma propriedade do Google Analytics a um anunciante do Display & Video 360 (Alfa)
...
Vinculação da conta do anunciante do Display & Video 360 (Alfa)
...
Saiba como configurar a integração do Search Ads 360 com o Google Analytics para uma propriedade do Google Analytics.
...
Gerenciamento de permissões de usuários (Alpha)
...
Saiba mais sobre os conjuntos de dados abertos no Google Analytics 360.
...
Saiba mais sobre como gerenciar o status de desativação do processo de configuração automática do Google Analytics para uma propriedade do UA.
...
Última atualização 2025-02-21 UTC.