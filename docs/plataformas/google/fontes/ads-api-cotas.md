...
Operações com o nível de acesso de teste	15.000 operações de API por dia em contas de teste	RESOURCE_EXHAUSTED
...
Operações com nível de acesso padrão	Operações de API diárias ilimitadas em contas de teste e de produção	N/A
Solicitações de mutação	10.000 operações de mutação por solicitação	TOO_MANY_MUTATE_OPERATIONS
...
Os limites diários de uso da API são baseados no número de operações de API feitas pelo seu projeto do Google Cloud. As operações de API são a soma total de solicitações Search e SearchStream, operações de mutação individuais e outras solicitações de serviço. Os limites para operações diárias de API dependem do nível de acesso à API do seu projeto do Google Cloud. O guia de níveis de acesso e uso permitido descreve os limites específicos de operação da API para cada nível de acesso.

As solicitações que violarem esses limites serão rejeitadas com o erro: RESOURCE_EXHAUSTED.
...
As solicitações que violarem essa limitação não vão gerar um GoogleAdsError, mas vão gerar um erro RESOURCE_EXHAUSTED (código gRPC 8 / HTTP 429). Consulte a lista de códigos e mensagens de erro do gRPC.

Solicitações de mutação

Além de contar na cota de operação diária do usuário, uma solicitação de mutação não pode conter mais de 10.000 operações por solicitação. As solicitações que violam essa limitação são rejeitadas com o erro: TOO_MANY_MUTATE_OPERATIONS.
...
Solicitações paginadas (por exemplo, solicitações que contêm um next_page_token válido) não são contabilizadas na cota de operação diária de um usuário. No entanto, as solicitações de paginação que contêm um token de página expirado ou inválido geram uma exceção e são contabilizadas na cota diária de operações.

Para mais detalhes sobre paginação, consulte Como fazer paginação de resultados.

Outros tipos de solicitações

Uma solicitação que não seja Mutate, Search ou SearchStream conta como uma operação na cota diária de operações do usuário.
...
As solicitações rejeitadas com um GoogleAdsFailure ainda contam na cota diária de operações do usuário.
...
Considere esses limites ao criar um plano de palavras-chave.
...
KeywordPlanCampaignKeyword (palavras-chave negativas) por KeywordPlan	1.000
KeywordPlanCampaign por KeywordPlan	1
Serviço de insights sobre público-alvo

Os seguintes métodos em AudienceInsightsService estão sujeitos a limites de cota específicos.

Limitado a aproximadamente 200 solicitações por dia e por CID:
...
As solicitações que violarem esses limites serão rejeitadas com o erro: TOO_MANY_CONVERSIONS_IN_REQUEST.

Observação: a partir de 15 de junho de 2026, o ConversionUploadService.UploadClickConversions será restrito para novos usuários (retornando NotAllowlistedError.CUSTOMER_NOT_ALLOWLISTED_FOR_THIS_FEATURE). As novas integrações de upload de conversão precisam usar a API Data Manager.
...
As solicitações que violarem esses limites serão rejeitadas com o erro: TOO_MANY_ADJUSTMENTS_IN_REQUEST.

Regras do valor da conversão

Limitado a 100.000 regras de valor da conversão por conta.

As solicitações que violarem esse limite serão rejeitadas com o erro ResourceCountLimitExceededError.ACCOUNT_LIMIT.
...
É necessário aguardar pelo menos 12 horas entre as mudanças no orçamento da conta (AccountBudget ou AccountBudgetProposal) na mesma conta. Fazer mudanças antes de 12 horas pode resultar em falhas irrecuperáveis que só podem ser resolvidas pelo representante da sua conta do Google Ads.

Convites para contas de clientes

Novos usuários podem ser convidados para contas de cliente atuais com o CustomerUserAccessInvitationService. Como esse recurso envia e-mails de convite para outros usuários, ele pode ser usado de forma indevida. Por isso, há limitações no comportamento dele:

Os usuários não podem receber mais de um convite pendente para a mesma conta de cliente. Se uma solicitação subsequente for feita para enviar um convite a um usuário que já tem um convite pendente, este erro será retornado: EMAIL_ADDRESS_ALREADY_HAS_PENDING_INVITATION.

As contas de cliente não podem ter mais de 70 convites pendentes por vez. Se uma solicitação for enviada e fizer com que esse valor seja excedido, este erro será retornado: PENDING_INVITATIONS_LIMIT_EXCEEDED.

Dados do usuário

Os dados do usuário são gerenciados com o UserDataService e o OfflineUserDataJobService.

Observação: a partir de 1º de abril de 2026, UserDataService e OfflineUserDataJobService para a segmentação por lista de clientes serão restritos para novos usuários (NotAllowlistedError.CUSTOMER_NOT_ALLOWLISTED_FOR_THIS_FEATURE que retornam). As novas integrações da segmentação por lista de clientes precisam usar a API Data Manager.
...
Por exemplo, se um usuário tiver 30 endereços de e-mail, você poderá enviar dois objetos UserData que compartilham um third_party_user_id comum (colocando thirdPartyUserId mais os e-mails de 1 a 19 no primeiro objeto para ficar dentro do limite de 20 identificadores e thirdPartyUserId mais os e-mails de 20 a 30 no segundo):
...
O limite total para user_identifiers em todas as operações em um único AddOfflineUserDataJobOperationsRequest é de 100.000 (um OfflineUserDataJob pode aceitar várias chamadas AddOfflineUserDataJobOperationsRequest, até um máximo recomendado de 1.000.000 de operações por job). Para uploads síncronos usando UploadUserDataRequest (UserDataService.UploadUserData), cada solicitação é limitada a um máximo de 10 operações e 100 user_identifiers em toda a solicitação.
...
Se você encontrar essa limitação e estiver fazendo solicitações que usam um campo repetido, tente reduzir o número de itens no campo repetido dividindo a lista de operações em várias solicitações de mutação.
